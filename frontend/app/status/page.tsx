...
import {
  Activity, Server, Cpu, Zap, Database,
...
function StatusIcon({ ok }: { ok: boolean | null }) {
  if (ok === null) return <Loader2 className="h-5 w-5 animate-spin text-[var(--muted-foreground)]" />;
  return ok
...
export default function StatusPage() {
  ...
  const fetchInfra = useCallback(async () => {
    try {
      const res = await fetch(`${API_BASE}/health/infra`, { credentials: "include" });
  ...
  const fetchMetrics = useCallback(async () => {
    try {
      const res = await fetch(`${API_BASE}/health/metrics`, { credentials: "include" });
  ...
  const testLLM = async () => {
    ...
    try {
      const start = performance.now();
      const res = await fetch(`${API_BASE}/health/llm`, { credentials: "include" });
        ...
        <div className="mx-auto max-w-3xl">
          ...
          <div className="mb-8 grid gap-4 sm:grid-cols-3">
            ...
            <div className="rounded-xl border border-[var(--border)] bg-[var(--card)]/40 p-5 transition-all hover:border-pawa-cyan/30 hover:shadow-lg hover:shadow-pawa-cyan/5">
              ...
              <div className="space-y-1.5 text-xs text-[var(--muted-foreground)]">
                <div className="flex justify-between">
                  <span>Status</span>
                  <span className={health ? "text-pawa-accent" : ""}>{health?.status || "checking..."}</span>
                </div>
                <div className="flex justify-between">
                  <span>Latency</span>
            ...
            <div className="rounded-xl border border-[var(--border)] bg-[var(--card)]/40 p-5 transition-all hover:border-pawa-cyan/30 hover:shadow-lg hover:shadow-pawa-cyan/5">
              ...
              <div className="space-y-1.5 text-xs text-[var(--muted-foreground)]">
                ...
                <div className="flex justify-between">
                  <span>Test</span>
                  {llmTestState === "idle" && (
                    <button onClick={testLLM} className="text-pawa-cyan hover:underline">Run test</button>
                  )}
                  {llmTestState === "testing" && (
                    <span className="flex items-center gap-1"><Loader2 className="h-3 w-3 animate-spin" /> testing...</span>
                  )}
                  {llmTestState === "done" && llmTest && (
                    <span className="text-pawa-accent">{llmTest.status} ({llmTest.latency_ms}ms)</span>
                  )}
                  {llmTestState === "error" && (
            ...
            <div className="rounded-xl border border-[var(--border)] bg-[var(--card)]/40 p-5 transition-all hover:border-pawa-cyan/30 hover:shadow-lg hover:shadow-pawa-cyan/5">
              ...
              <div className="space-y-1.5 text-xs text-[var(--muted-foreground)]">
                <div className="flex justify-between">
                  <span>Mode</span>
                  <span className={health?.rust_native ? "text-pawa-accent" : ""}>
                    {health ? (health.rust_native ? "Native PyO3" : "Python fallback") : "—"}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>Functions</span>
                ...
                {!health?.rust_native && health && (
                  <div className="flex items-start gap-1 mt-1 text-[10px] text-pawa-warning">
                    ...
                    <span>Rust binary not compiled — using Python fallback</span>
                  </div>
                )}
              </div>
            </div>
          ...
          {infra && (
            <div className="mb-8 rounded-xl border border-[var(--border)] bg-[var(--card)]/40 p-5">
              <h3 className="text-sm font-medium mb-3 flex items-center gap-2">
                <Server className="h-4 w-4 text-pawa-cyan" />
                {infra.environment === "fly_io" ? "Fly.io Infrastructure" : infra.environment === "cloud_run" ? "Cloud Run Infrastructure" : "Local Environment"}
              </h3>
              <div className="grid gap-2 text-xs text-[var(--muted-foreground)] sm:grid-cols-2">
                ...
                {infra.environment === "fly_io" && (
                  <>
                    ...
                    <div className="flex justify-between">
                      <span>Alloc ID</span>
                      <span className="font-mono text-[10px]">{infra.fly_io.alloc_id}</span>
                    </div>
                  </>
                )}
                {infra.environment === "cloud_run" && (
                  <>
                    ...
                    <div className="flex justify-between">
                      <span>Revision</span>
                      <span className="font-mono text-[10px]">{infra.cloud_run.revision}</span>
                    </div>
                  </>
                )}
              </div>
          ...
          {metrics && metrics.benchmarks.length > 0 && (
            <div className="mb-8 rounded-xl border border-[var(--border)] bg-[var(--card)]/40 p-5">
              <h3 className="text-sm font-medium mb-3 flex items-center gap-2">
                <Zap className="h-4 w-4 text-pawa-cyan" />
                PyO3 Performance — {metrics.rust_native ? "Rust Native" : "Python Fallback"}
              </h3>
              <div className="space-y-3">
                {metrics.benchmarks.map((b) => (
                  <div key={b.function} className="flex items-center justify-between text-xs">
                    <div>
                      <span className="font-mono text-[var(--foreground)]">{b.function}</span>
                      <span className="ml-2 text-[var(--muted-foreground)]">({b.payload_bytes}B payload)</span>
                    </div>
                    <div className="flex items-center gap-3">
                      <div className="h-1.5 w-24 overflow-hidden rounded-full bg-[var(--muted)]">
                        <div
                          ...
                          style={{ width: `${Math.min(100, Math.max(5, 100 - b.avg_us))}%` }}
                        />
                      </div>
                      <span className={cn("font-mono tabular-nums", b.avg_us < 5 ? "text-pawa-accent" : "text-[var(--foreground)]")}>
              ...
              {metrics.redis.status === "connected" && (
                <div className="mt-4 border-t border-[var(--border)] pt-3">
                  <div className="grid grid-cols-2 gap-2 text-xs text-[var(--muted-foreground)] sm:grid-cols-4">
                    <div>
                      <span className="block text-[10px] uppercase tracking-wider">Uptime</span>
                      <span className="font-mono">{Math.floor((metrics.uptime_seconds) / 3600)}h {Math.floor((metrics.uptime_seconds % 3600) / 60)}m</span>
                    </div>
                    <div>
                      <span className="block text-[10px] uppercase tracking-wider">Redis Memory</span>
                      <span className="font-mono">{metrics.redis.used_memory_mb}MB</span>
                    </div>
                    <div>
                      <span className="block text-[10px] uppercase tracking-wider">Redis Keys</span>
                      <span className="font-mono">{metrics.redis.total_keys}</span>
                    </div>
                    <div>
                      <span className="block text-[10px] uppercase tracking-wider">Redis Version</span>
          ...
          {metrics?.postgres?.status === "connected" && (
            <div className="mb-8 rounded-xl border border-[var(--border)] bg-[var(--card)]/40 p-5">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs text-[var(--muted-foreground)] sm:grid-cols-4">
                <div>
                  <span className="block text-[10px] uppercase tracking-wider">Version</span>
                  <span className="font-mono">{metrics.postgres.version}</span>
                </div>
                <div>
                  <span className="block text-[10px] uppercase tracking-wider">DB Size</span>
                  <span className="font-mono">{metrics.postgres.database_size}</span>
                </div>
                <div>
                  <span className="block text-[10px] uppercase tracking-wider">Conversations</span>
                  <span className="font-mono">{metrics.postgres.conversations}</span>
                </div>
                <div>
                  <span className="block text-[10px] uppercase tracking-wider">Users</span>
                  <span className="font-mono">{metrics.postgres.users}</span>
                </div>
                <div>
                  <span className="block text-[10px] uppercase tracking-wider">Pool Size</span>
                  <span className="font-mono">{metrics.postgres.pool_size}</span>
                </div>
                <div>
                  <span className="block text-[10px] uppercase tracking-wider">Pool Idle</span>
                  <span className="font-mono">{metrics.postgres.pool_idle}</span>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="mb-8 rounded-xl border border-dashed border-[var(--border)] bg-[var(--card)]/20 p-5">
            ...
            <ul className="space-y-2 text-xs text-[var(--muted-foreground)]">
              <li className="flex items-start gap-2">
                ...
                <span><strong>Observability</strong> — ELK stack for structured log aggregation, Prometheus exporters for request metrics, Grafana dashboards for latency percentiles and error rates</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="mt-0.5 h-1.5 w-1.5 shrink-0 rounded-full bg-pawa-warning/60" />
                <span><strong>APM</strong> — OpenTelemetry traces across FastAPI → Gemini → Redis, distributed tracing with Cloud Trace, SLO alerting on p99 latency</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="mt-0.5 h-1.5 w-1.5 shrink-0 rounded-full bg-pawa-warning/60" />
                <span><strong>CI/CD</strong> — GitHub Actions pipeline with parallel lint/test/build, preview deployments per PR, automated Lighthouse audits</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="mt-0.5 h-1.5 w-1.5 shrink-0 rounded-full bg-pawa-warning/60" />
                <span><strong>Multi-turn context</strong> — conversation_id parameter already in schema, wire to Gemini chat sessions for follow-up questions</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="mt-0.5 h-1.5 w-1.5 shrink-0 rounded-full bg-pawa-warning/60" />
                <span><strong>RAG pipeline</strong> — embed GCP docs with Vertex AI, store in AlloyDB pgvector, retrieval-augmented generation for grounded answers</span>
              </li>
            </ul>
          </div>
